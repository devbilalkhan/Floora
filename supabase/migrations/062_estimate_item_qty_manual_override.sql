-- Tracks whether a user has directly edited the qty of an auto-managed
-- consumable row (Weld Rod, Glue, Feather Finish, Carpet Underlay, etc).
-- Once true, the app skips recalculating that row's qty from its parent
-- primary's qty / section area formula — previously any parent qty edit
-- silently overwrote a manually corrected consumable qty.
alter table estimate_items
  add column qty_manually_set boolean not null default false;

-- Keep duplicate_estimate() in sync with the new column so duplicated
-- estimates preserve manual overrides instead of reverting to auto.
...
begin
  ...
  insert into estimate_items (
    ...
    manufacturer, level, product_type, qty_manually_set
  )
  select
    ...
    ei.manufacturer, ei.level, ei.product_type, ei.qty_manually_set
