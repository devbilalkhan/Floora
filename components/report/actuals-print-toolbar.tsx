...
export function ActualsPrintToolbar({ projectName }: { projectName: string }) {
  return (
    <div className="fixed top-0 left-0 right-0 h-12 bg-zinc-800/95 backdrop-blur border-b border-zinc-700 flex items-center px-5 gap-3 z-50 print:hidden">
      <span className="text-xs font-semibold text-zinc-200">Actuals Report</span>
      <span className="text-xs text-zinc-600">—</span>
      <span className="text-xs text-zinc-400 truncate max-w-xs">{projectName}</span>
