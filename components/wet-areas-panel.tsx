...
import { computeWetAreaLabor, type WetArea } from "@/lib/estimate-types";
...
const ro =
  "w-full px-1.5 py-1 text-[11px] tabular-nums text-right text-foreground/60 select-none cursor-default";
...
function WetAreaRow({
  ...
  const floorLab = (Number(local.floor_sqm)     || 0) * LAB.vinyl;
  const semiLab  = (Number(local.wall_semi_sqm) || 0) * LAB.wallVinylSemi;
  const fullLab  = (Number(local.wall_full_sqm) || 0) * LAB.wallVinylFull;
  const covLab   = (Number(local.coving_lm)     || 0) * LAB.coving;
  const unitLab  = computeWetAreaLabor(local);
    ...
    <tr className="border-b border-border hover:bg-muted/20 group">
      ...
      <td className="border-r border-border p-0">
        <input
          ...
          className="w-full px-2 py-1 text-[11px] bg-transparent border-0 outline-none focus:ring-1 focus:ring-inset focus:ring-primary/40 text-foreground/70 placeholder:text-muted-foreground/20"
      ...
      {/* Floor Lab (per area) */}
      <td className="border-r border-border p-0 bg-muted/10">
      ...
      <td className="border-r border-border p-0 bg-primary/5">
        <div className="w-full px-1.5 py-1 text-[11px] tabular-nums text-right font-semibold text-foreground/80 select-none cursor-default">
      ...
      <td className="border-r border-border p-0">
        <div className={cn(
          ...
          local.charge === 0 && unitLab === 0 ? "text-muted-foreground/25"
...
export function WetAreasPanel({
  ...
  const totalLab    = areas.reduce((s, a) => s + computeWetAreaLabor(a) * (Number(a.qty) || 1), 0);
  ...
  return (
    <div className="border border-border rounded-xl overflow-hidden bg-card/65 backdrop-blur-xl">
      {/* Header */}
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border bg-muted/30">
        ...
        <div className="ml-auto flex items-center gap-4 text-[10px] text-muted-foreground/50">
          <span>Floor $23/m²</span>
          <span>Wall Semi $29/m²</span>
          <span>Wall Full $35/m²</span>
          <span>Coving $25/lm</span>
        </div>
      </div>
            ...
            <tr className="bg-muted/40 border-b border-border">
              {([
                ...
                <th
                  key={i}
                  className={cn(
                    "px-2 py-1 text-[9px] font-medium text-muted-foreground uppercase tracking-wide border-r border-border last:border-r-0",
                    align,
                    shaded && "bg-muted/20"
                  )}
                >
          ...
          <tbody>
            ...
            {areas.length > 0 && (
              <tr className="bg-muted/25 border-t-2 border-border">
                ...
                <td className="border-l border-border px-1.5 py-2 text-right text-[11px] tabular-nums font-semibold text-foreground/80">
                  ${fmt(totalLab)}
                </td>
                <td className="border-l border-border" /> {/* Qty — no total */}
                <td className="border-l border-border px-1.5 py-2 text-right text-[11px] tabular-nums font-semibold text-foreground/80">
                  ${fmt(totalCharge)}
                </td>
                <td className={cn(
                  "border-l border-border px-1.5 py-2 text-right text-[11px] tabular-nums font-bold",
