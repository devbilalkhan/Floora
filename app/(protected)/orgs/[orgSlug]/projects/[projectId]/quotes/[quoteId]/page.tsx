...
export default async function SavedQuotePage({
  ...
  const [{ data: quote }, { data: project }, { data: org }] = await Promise.all([
    supabase.from("quotes").select("id, org_id, project_id, estimate_id, quote_number, quote_date, valid_for, company_name, company_abn, company_address, company_phone, company_email, to_name, to_address, to_contact, to_email, project_ref, project_loc, scope_text, notes, terms, lines, total_ex_gst, gst, grand_total, status, created_at, updated_at").eq("id", params.quoteId).single(),
