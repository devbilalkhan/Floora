...
function gpBadgeStyle(pct: number, hasItems: boolean): string {
  if (!hasItems) return "bg-muted text-muted-foreground border-border";
...
export function EstimateTableRow({
  ...
  return (
    <tr
      className="group border-b border-border/60 hover:bg-muted/30 transition-colors cursor-pointer last:border-0"
      onClick={() => router.push(href)}
    >
      <td className="px-3 py-1.5 font-medium text-foreground/80 group-hover:text-primary transition-colors">
        {estimate.name}
      </td>
